# Kapitel 4.4.2: TMDB-Integration

Automatische Film- und TV-Serien-Metadaten für DVDs und Blu-rays via The Movie Database API.
...
- **Keine Kosten** für non-commercial use
...
| **Mehrfach-Treffer** | Häufig (Pressungen) | Gelegentlich (Remakes) |
| **Datenqualität** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
...
4. Klicke **"Sign Up"**
5. Bestätige Email-Adresse

#### 2. API-Key beantragen

1. Logge dich bei TMDB ein
2. Navigiere zu **Settings** (Profilbild → Dropdown)
3. Wähle **"API"** im linken Menü
4. Klicke **"Request an API Key"** oder **"Create"**
5. Wähle Typ: **"Developer"**
...
2. Zwei Keys werden angezeigt:
   - **API Key (v3 auth)**: ← Dieser wird benötigt
...
**Kapazität**: ~800 Discs/10 Minuten
...
extract_movie_title() {
    ...
    # 5. Artikel "the" → Anfang verschieben
    title=$(echo "$title" | sed -E 's/^the //')
    title="the $title"
    
    # 6. Capitalize (erster Buchstabe jedes Worts groß)
...
**Pattern** für Staffel/Disc-Nummer:
...
- `language`: Sprache für Metadaten (z.B. `de-DE`, `en-US`)
  ...
  "results": [
    {
      ...
      "overview": "Der Chemielehrer Walter White erfährt, dass er...",
      "vote_average": 8.9
    }
  ]
}
...
→ TV-Serie: "Sherlock" (2010)
→ Type-Selection Modal erscheint
```
...
<div id="type-selection-modal" class="modal">
  <h2>Content-Typ wählen für "Sherlock"</h2>
...
3. **Ähnliche Titel**: "Avatar" (2009) vs. "Avatar: The Last Airbender" (2010)
4. **Dokumentationen**: "The Beatles" → 50+ Ergebnisse
...
<div id="tmdb-modal" class="modal modal-large">
  <h2>Film auswählen (15 Treffer)</h2>
...
function selectMovie(index) {
    ...
    .then(() => {
        hideModal();
        showNotification('Film ausgewählt, Metadaten werden erstellt...');
    });
}
```
...
- `w185`: 185px
- `w342`: 342px
- `w500`: 500px ← **disk2iso Standard**
- `w780`: 780px
- `original`: Originalgröße (oft >2000px)
...
download_tmdb_poster() {
    ...
    if [[ -z "$poster_path" || "$poster_path" == "null" ]]; then
        log_warning "Kein Poster verfügbar"
        return 1
    fi
...
**Wenn kein Poster verfügbar**:

```bash
# Placeholder kopieren
cp /opt/disk2iso/www/static/img/dvd-placeholder.png \
...
**Lösung**:
- disk2iso enthält Rate-Limiting (max 3 req/disc)
...
- Generische Labels: "DISC_1" → keine sinnvolle Suche

**Lösung**:
1. **Manuelle Suche** auf themoviedb.org
2. **Manuelle Metadaten** im Web-Interface eingeben
3. **Nachträgliche Erfassung** nach TMDB-Hinzufügung
...
```bash
# In lib/config.sh
grep "TMDB_LANGUAGE" /opt/disk2iso/lib/config.sh
```
...
**Verfügbare Sprachen**: de-DE, en-US, fr-FR, es-ES, it-IT, ...

---

## Weiterführende Links

- **[← Zurück: Kapitel 4.4.1 - MusicBrainz](04-4-1_MusicBrainz.md)**
- **[Kapitel 4.2: DVD-Video Modul →](../04-2_DVD-Video.md)**
- **[Kapitel 4.3: Blu-ray-Video Modul →](../04-3_BD-Video.md)**
